...
for (const correctRows of rows) {
  const [name, from, to, time] = correctRows.split(';');
  const correctName = name.slice(1).replace('_', ' ');
  const correctFrom = from.slice(0, 3).toUpperCase();
  const correctTo = to.slice(0, 3).toUpperCase();

  const correctString = `${correctName} from ${correctFrom} to ${correctTo} (${time})`;

  if (correctName.startsWith('Delayed')) {
    console.log(`🔴` + `${correctString}`.padStart(42));
  } else {
    console.log(correctString.padStart(44));
  }
}
